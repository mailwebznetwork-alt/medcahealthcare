<x-operations.workspace>
    ...
    <form method="get" action="{{ route('operations.job-portal.applications.index') }}" class="mb-8 flex flex-wrap gap-3">
        <select name="vacancy_id" class="min-w-[14rem] mom-control-pill border-[rgba(255,255,255,0.045)] bg-[rgba(28,22,18,0.75)] px-3 py-2.5 text-sm text-[var(--text-primary)] shadow-mom-inner">
            <option value="">{{ __('All vacancies') }}</option>
            @foreach ($vacancies as $v)
                <option value="{{ $v->id }}" @selected(request('vacancy_id') == $v->id)>{{ $v->title }}</option>
            @endforeach
        </select>
        <select name="pipeline_status" class="mom-control-pill border-[rgba(255,255,255,0.045)] bg-[rgba(28,22,18,0.75)] px-3 py-2.5 text-sm text-[var(--text-primary)] shadow-mom-inner">
